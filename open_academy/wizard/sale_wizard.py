...
class SaleWizard(models.TransientModel):
	...
	session_student_ids=fields.Many2many(comodel_name="res.partner",
		...
		help='These are the students Currently in the Session Students')

	student_ids=fields.Many2many(comodel_name='res.partner',
		string='Students for Sale orders')
		...
		if session_product_id:
				for student in self.student_ids:
					order_id=self.env['sale.order'].create({
						...
						'order_lines':[(0,0,{'product_id':session_product_id.id, 	'price_unit':self.session_id.total_price})]
						})		