    ...
    def action_create_redemption(self):
        ...
        if self.points < 1:
            ...
            raise UserError(_('%s have only %s points for redemption', customer_invoice.partner_id.name,customer_invoice.partner_id.partner_balance_points))
             ...
             max_redeemeble_points = customer_invoice.amount_total * self.env.company.store_loyalty_points / self.env.company.store_value_for_every_point
             if self.points < max_redeemeble_points:
                 customer_invoice.partner_id.partner_balance_points -= self.points
                 price_deduct = self.points * self.env.company.store_value_for_every_point / self.env.company.store_loyalty_points
             if self.points >= max_redeemeble_points:
                 customer_invoice.partner_id.partner_balance_points -= max_redeemeble_points
                 price_deduct = max_redeemeble_points * self.env.company.store_value_for_every_point / self.env.company.store_loyalty_points
             invoice_line = {'name': 'redeem loyalty pointys', 'product_id': self.env.company.store_product, 'price_unit': -price_deduct, 'price_subtotal': -price_deduct}
