...
owl_odoo_shop = Flask(__name__)
owl_odoo_shop.config['order_id'] = 1
...
@owl_odoo_shop.route('/search', methods=['GET', 'POST'])
def search_product():
    res = "Invalid Session"
...
def get_new_order_id():
    owl_odoo_shop.config['order_id'] += 1
    return owl_odoo_shop.config['order_id']
...
@owl_odoo_shop.route('/add_to_cart', methods=['GET', 'POST'])
def add_to_cart():
    if is_valid_session():
        qty = 1
...
@owl_odoo_shop.route('/remove_from_cart', methods=['GET', 'POST'])
...
@owl_odoo_shop.route('/checkout', methods=['GET', 'POST'])
def checkout():
