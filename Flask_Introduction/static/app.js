...
class App extends Base{
	template = "<div><div id='topbar'></div><div id='left_panel' style='float: left;'><h2>Product List</h2></div><div id='right_panel' style='float: right;'></div></div>";
...
class Cart extends Base{
    ...
    render_checkout () {
        ...
        for (var i=0; i < this.order.products.length; i++) {
            this.template = `<div>${this.order['products'][i].name}, ${this.order['products'][i].qty}, ${this.order['products'][i].price}</div>`;            
            super.render();
        }
