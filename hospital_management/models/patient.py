...
class HospitalPatient(models.Model):
        ...
        #BASIC FIELDS
        name = fields.Char(string='Name',tracking=True,required=True,placeholder="Enter Your Name")
        reference = fields.Char(string="Order.reference",copy=False,
                                readonly=True)
        age = fields.Integer(string='Age',tracking=True,required=True,size=3)
        ...
        date_of_admit = fields.Date(string="Date of Admit")
        date_of_discharge = fields.Datetime(string="Date of Discharge")
        note = fields.Text(string='Description')
        color = fields.Integer(string="Color_Box")

        #RELATIONAL FIELDS
        partner_id = fields.Many2one('res.partner', string='Parent_Name')
        doctor = fields.Many2many("hospital.doctor","patient_doctor_rel","patient_id","doctor_id",string='Doctor Name')
        bill = fields.One2many("hospital.bill","p_name",string='Bill Amount',ondelete="restrict")
