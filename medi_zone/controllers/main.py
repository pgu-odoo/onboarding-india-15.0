...
class MediOrderController(Controller):

    def get_product(self, pid):
        return request.env['product.product'].search([('id', '=', int(pid))])

    def get_current_order(self):
        return request.env['medi.order'].search([('state', '=', 'draft')])

    def get_order_line(self, order, product_id):
        return request.env['medi.order.line'].search([('order_id', '=', order.id), ('product_id', '=', product_id)])
    ...
    def add_order_line(self, order, product, qty):
        ...
        return request.env['medi.order.line'].create(vals)

    def add_order(self):
        vals = {
            'partner_id': request.env.user.id
        }
        return request.env['medi.order'].create(vals)
    ...
    def search_product(self, name):
        ...
        products = request.env['product.product'].search([('name', 'ilike', name)])
        for p in products:
            vals = {
                'id': p.id,
                'name': p.name,
